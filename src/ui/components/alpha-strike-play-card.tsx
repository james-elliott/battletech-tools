...
export default class AlphaStrikeUnitCard extends React.Component<IAlphaStrikeUnitCardProps, IAlphaStrikeUnitCardState> {
    ...
    private _toggleEngineHit = ( reset: boolean = false ) => {
        if(this.props.asUnit ) {
            ...
            if (this.props.asUnit.getEngineHits(true) < this.props.asUnit.roundEngineHits.length) {
                this.props.asUnit.roundEngineHits[this.props.asUnit.getEngineHits(true)] = true;
                
            } else if (reset) {
                for (let index = 0; index < this.props.asUnit.roundEngineHits.length; index++) {
                    this.props.asUnit.roundEngineHits[index] = false;
                }
            }
    ...
    private _toggleWeaponHit = ( reset: boolean = false ) => {
        if(this.props.asUnit ) {
            ...
            if (this.props.asUnit.getWeaponHits(true) < this.props.asUnit.roundWeaponHits.length) {
                this.props.asUnit.roundWeaponHits[this.props.asUnit.getWeaponHits(true)] = true;
            } else if (reset) {
                for (let index = 0; index < this.props.asUnit.roundWeaponHits.length; index++) {
                    this.props.asUnit.roundWeaponHits[index] = false;
                }
            } else {
                this._takeDamage(1);
            }
    ...
    private _clearChanges = (): void => {
        if (this.props.asUnit) {
            ...
            this.props.asUnit.roundEngineHits = [];
    ...
    private _moveOptions = (unit: AlphaStrikeUnit): JSX.Element => {
        ...
        for (let moveC of unit.move) {
            if (moveC.type !== 'j' && moveC.type !== 's' && moveC.type !== 'v' && moveC.currentMove > 0 || (moveC.type === 'j' && unit.move.length === 1)) {
                ground.push(
            ...
            if (moveC.type === 'v') {
                airborne.push(
                    <div key="standstill" className='token-wrapper' onClick={() => this._setMovement('standstill', 2)}>
                        {this._moveToken('standstill', 2)}
                    </div>
                );
                airborne.push(
                    <div key="ground" className='token-wrapper' onClick={() => this._setMovement('ground', moveC.tmm)}>
        ...
        // console.log(unit.name, ground.length);
        return <>
    ...
    render = (): JSX.Element => {
        ...
        // console.log(this.props.asUnit.name, this.props.asUnit);
                    ...
                    {!this.props.asUnit.isInfantry && this.props.asUnit.type !== "JS" ? (
                        <div className='crits column justified'>
                            ...
                            {this.props.asUnit.type.toLowerCase() !== "pm" ? (
                                <a className={this.props.asUnit.getEngineHits() !== this.props.asUnit.getEngineHits(true) ? 'data-pair row justified staged' : 'data-pair row justified'} onClick={() => this._toggleEngineHit(true)}><span>Engine</span>{this.props.asUnit.getEngineHits() !== this.props.asUnit.getEngineHits(true) ? this.props.asUnit.getEngineHits(true) : this.props.asUnit.getEngineHits()}</a>
                            ) : null }
                            <a 
                            ...
                            <a 
                                className={this.props.asUnit.getWeaponHits() !== this.props.asUnit.getWeaponHits(true) ? 'data-pair row justified staged' : 'data-pair row justified'} 
                                onClick={() => this._toggleWeaponHit(true)}
                                >
                                    <span>Weapons</span>
                                    {this.props.asUnit.getWeaponHits() !== this.props.asUnit.getWeaponHits(true) ? this.props.asUnit.getWeaponHits(true) : this.props.asUnit.getWeaponHits()}
                            </a>
                            ...
                            {this.props.asUnit.type.toLowerCase() === 'cv' ||  this.props.asUnit.type.toLowerCase() === 'sv'? (
                                <div className='vehicle data-pair row justified'>
                                    <span>Motive</span>
                                    <a className={roundMotive910 !== motive910 ? 'staged' : ''} onClick={() => this._toggleVehicle910()}>{roundMotive910 !== motive910 ? roundMotive910 : motive910}</a>
                                    <a className={roundMotive11 !== motive11 ? 'staged' : ''} onClick={() => this._toggleVehicle11()}>{roundMotive11 !== motive11 ? roundMotive11 : motive11}</a>
                                    <a className={this.props.asUnit.vehicleMotive12 !== this.props.asUnit.roundVehicleMotive12 ? 'staged' : ''} onClick={() => this._toggleVehicle12()}>{motive12}</a>
                                </div>
                            ) : null }
...
export class AlphaStrikeAttackOverlay extends React.Component<AlphaStrikeAttackOverlayProps, AlphaStrikeAttackOverlayState> {
    ...
    private tmmRange = this.props.attack.type === 'a2a' ? [-2,-1,1,2] : [-4,-2,-1,0,1,2,3,4,5];
    private toHitRollResults: RollResult[] = [];
    private damageRollResults: RollResult[] = [];
    private totalDamage = 0;
    // private effectOptions = ['FLAK','HT','LRM','SRM','AC']
    ...
    private _calcAttack = (): void => {
        if (this.props.unit && this.props.attack) {
            ...
            if (this.props.appGlobals.appSettings.alphaStrikeVariableDamage === '' || this.props.attack.type === 'physical') {
                if (this.toHitRollResults.length > 0 && this.toHitRollResults[0].hit) {
                    ...
                    if (this.state.minimal) {
                        this.totalDamage += this.damageRollResults[0].hit ? 1 : 0;
                    }
                }
            } else
            ...
            if (this.props.appGlobals.appSettings.alphaStrikeVariableDamage === 'attack' && this.toHitRollResults.length > 0) {
                for(let index = 0; index < this.maxDamage; index++) {
                    this.totalDamage += this.toHitRollResults[index].hit ? 1 : 0;
                }
            ...
            // Reduce damage with specials, like AMS
        }
    }
    ...
    private _rollAttack = (): void => {
        if (this.props.unit) {
            ...
            if (this.props.appGlobals.appSettings.alphaStrikeVariableDamage === 'damage') {
                ...
                for(let roll of attackRollResults) {
            ...
            this.setState({
                ...
                // attackRollResults: attackRollResults,
    ...
    private _showDamageDice = (): boolean => {
        let show = false;
        if (this.toHitRollResults.length > 0) {
            if (this.props.appGlobals.appSettings.alphaStrikeVariableDamage === 'damage') {
                // When not using Multiple Damage Rolls, the first hit needs to be successful to show damage
                show = this.toHitRollResults[0].hit;
            } else if (this.state.minimal) {
                // Show if we have minimal damage
                show = this.toHitRollResults[this.toHitRollResults.length - 1].hit;
            }
        }

        // Never show damage with Artillery or Bombs
        if (this.state.artillery || this.state.bombs.max > 0 || this.props.attack.type === 'physical') {
            show = false;
        } 
        ...
        return <>
            <div className='overlay column'>
                ...
                {this._showDamageDice() ? (
                    <>
                        <div className='row'>
                            {this.damageRollResults.map( (result,resultIndex) => {
